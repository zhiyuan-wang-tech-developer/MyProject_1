 ...
 * $Revision: 6268 $ (v4.040.05.011646)
 * $Date: 2016-10-04 17:14:11 +0530 (Tue, 04 Oct 2016) $
 *
 */
#include <ph_Status.h>
#include <phOsal.h>
...
static volatile uint32_t gdwEvents[PH_OSAL_CONFIG_NULLOS_MAX_NUM_EVENTS];
static volatile uint32_t gdwEventBitMap;
static phOsal_Event_t gCurrentEventHandle;
...
phStatus_t phOsal_Init(void)
{
    gbWaitTimedOut = 0;
    gdwEventBitMap = 0;
    gCurrentEventHandle = 0;

    return phPlatform_InitTickTimer(&phOsal_NullOsSysTickHandler);
}
...
phStatus_t phOsal_EventCreate(phOsal_Event_t *eventHandle, pphOsal_EventObj_t eventObj)
{
    ...
    PH_CHECK_SUCCESS(phOsal_NullOs_GetFreeIndex(&bEventIndex, gdwEventBitMap, PH_OSAL_CONFIG_NULLOS_MAX_NUM_EVENTS));
    ...
    phOsal_EventBits_t FlagsToWait, phOsal_EventBits_t *pCurrFlags)
{
    ...
    {
        phPlatform_StartTickTimer(ticksToWait);
        gCurrentEventHandle = *eventHandle;
    }
...
//        phPlatform_Sleep();
    ...
    return PH_ADD_COMPCODE(status, PH_COMP_OSAL);
}
...
phStatus_t phOsal_EventDelete(phOsal_Event_t * eventHandle)
{
	if((eventHandle == NULL) || ((*eventHandle) == NULL))
    {
        return PH_ADD_COMPCODE(PH_ERR_EVENT_ERROR, PH_COMP_OSAL);
    }
...
{
    uint32_t dwUnit[] = {PH_PLATFORM_TIMER_UNIT_MS, PH_PLATFORM_TIMER_UNIT_S};

    if ((timerHandle == NULL) || (timerObj == NULL) || (timerObj->timePeriod.unitPeriod > PH_PLATFORM_TIMER_UNIT_S))
    {
        return (PH_ERR_INVALID_PARAMETER | PH_COMP_OSAL);
    }

    phPlatform_Timer_Create(gpphPlatform_DataParams, dwUnit[timerObj->timePeriod.unitPeriod], (phPlatform_Timer_Struct_t **)(&timerObj->TimerHandle));
    *timerHandle = &timerObj->TimerHandle;

    return phPlatform_Timer_Configure(timerObj->TimerHandle, timerObj->timePeriod.period, (pphPlatform_CallBck_t)(timerObj->timerCb));
}

phStatus_t phOsal_TimerStart(phOsal_Timer_t * timerHandle)
{
    return phPlatform_Timer_Start(*timerHandle, ((pphOsal_TimerObj_t)timerHandle)->opt);
}

phStatus_t phOsal_TimerStop(phOsal_Timer_t * timerHandle)
{
    return phPlatform_Timer_Stop(*timerHandle);
}

phStatus_t phOsal_TimerGetCurrent(phOsal_Timer_t * timerHandle, uint32_t * pdwGetElapsedTime)
{
    return phPlatform_Timer_GetElapsedDelay(*timerHandle, pdwGetElapsedTime);
}

phStatus_t phOsal_TimerModify(phOsal_Timer_t *timerHandle, pphOsal_TimerObj_t timerObj)
{
    return phPlatform_Timer_Configure(*timerHandle, timerObj->timePeriod.period, (pphPlatform_CallBck_t)(timerObj->timerCb));
}

phStatus_t phOsal_TimerDelete(phOsal_Timer_t * timerHandle)
{
    return phPlatform_Timer_Delete((phPlatform_Timer_Struct_t *)(*timerHandle));
}
