...
#warning "Number of Hardware Timers are NOT matching with the PH_PLATFORM_MAX_TIMERS."
#endif
...
{
    uint32_t dwElapsedCnt;
//    uint32_t dwTimerFreq = 1;
//
//     dwTimerFreq = phPlatform_Port_GetTimerFreq(pTimerHandle);
//
...
//     *pdwGetElapsedDelay = (uint32_t)((dwElapsedCnt * dwUnits)/dwTimerFreq) ;
    ...
    dwElapsedCnt = (uint32_t)TMR1_SoftwareCounterGet();
    *pdwGetElapsedDelay = dwElapsedCnt;
...
//    qwLoadValue = ((uint64_t)dwTimeMilliSecs * (uint64_t)dwSysTickTimerFreq)/PH_PLATFORM_TIMER_UNIT_MS;
      /* Start the tick timer with the loaded tick value */
      qwLoadValue = (uint64_t)dwTimeMilliSecs;
...
/* Every 1 ms the Timer 1 ISR is triggered and this System Tick Handler is called. */
...
//    }
    ...
    /* Check the match between the loaded tick value and the Timer 1 software counter value. */
    if(qwLoadValue == TMR1_SoftwareCounterGet())
    {
        /* Stop the Timer and call the callBack. */
        phPlatform_StopTickTimer();
        /* This function pointer actually points to phOsal_NullOsSysTickHandler() in phOsal_NullOs.c file. */
        if( pTickCallBack != NULL ){
            pTickCallBack();       
        }
    }
}
